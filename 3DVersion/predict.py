...
def adjust_index_for_boundary(
    x: int, img_shape: tuple, window_shape: tuple, dim: int
) -> int:
    """
...
def crop_batch(batch, start, end, window_size, stride, img_shape):
    ...
    return batch[
        ...,
        shifted_start[0] : shifted_end[0],
        shifted_start[1] : shifted_end[1],
        shifted_start[2] : shifted_end[2],
    ]
...
def predict_stack(
    ...
    with torch.no_grad():
        ...
        for z in tqdm(
            range(0, img_shape[-3] - stride[0] + stride[0], stride[0]), disable=silent
        ):
            z = adjust_index_for_boundary(z, img_shape, img_window, 0)
            for x in tqdm(
                range(0, img_shape[-2] - img_window[1] + stride[1], stride[1]),
                leave=False,
                disable=silent,
            ):
                ...
                for y in range(0, img_shape[-1] - img_window[2] + stride[2], stride[2]):
                    ...
                    pred_batch = model.predict(img_batch[None, None])[0]
                    pred_batch = crop_batch(
                        pred_batch,
                        top_left,
                        bottom_right,
                        img_window,
