...
def UNetFactory(ConvLayer):
    class DownLayer(CustModule):
        ...
        def __init__(
            ...
            super().__init__(type)
            self.down_pool = self.MaxPool(
                kernel_size=self.pool_down_kernel(depth_downsample)
            )
            self.conv_layer = ConvLayer(
                in_c, out_c, type, residual=residual, cat_emb_dim=cat_emb_dim
            )
    ...
    class Encoder(CustModule):
        def __init__(
            ...
            self.depth_downsample = (
                depth_downsample
                if (type == "3D" and depth_downsample is not None)
                else [True] * (len(channels) - 2)
            )
    ...
    class Decoder(CustModule):
        def __init__(
            ...
            self.enc_channels = enc_channels[
                -2::-1
            ]  # Reverse and exclude the last entry
            self.depth_upsample = (
                depth_upsample[::-1]
                if (type == "3D" and depth_upsample is not None)
        ...
        def forward(
            self, features: torch.Tensor, cat_embedding: Optional[torch.Tensor] = None
        ) -> torch.Tensor:
            """
    ...
    class UNet(nn.Module):
        ...
        def _check_args(
            self, encoder_channels, decoder_channels, type, depth_downsample
        ):
            ...
            if type == "2D":
                assert (
                    depth_downsample is None
                ), "If type is 2D, there is no depth downsampling possibility!"
...
class OGConvLayer(CustModule):
    def __init__(
        ...
        # Category embedding
        self.mlp_cat = (
            nn.Linear(cat_emb_dim, 2 * out_c) if cat_emb_dim is not None else None
        )
    ...
    def forward(self, x, cat_embedding=None):
        ...
        if cat_embedding is not None:
            ...
            cat_embedding = rearrange(
                cat_embedding, "b c -> b c 1 1 1"
            )  # Just broadcast over all spatial dimensions
            scale_shift = cat_embedding.chunk(2, dim=1)
        else:
            scale_shift = None

        x = self.relu(
            self.add_category_embedding(self.norm1(self.conv1(x)), scale_shift)
        )
        y = self.relu(
            self.add_category_embedding(self.norm2(self.conv2(x)), scale_shift)
        )
        if self.residual:
            return x + y
        else:
...
class ConvNeXtLayer(CustModule):
    def __init__(
        ...
        super().__init__(type)
        self.conv1 = self.Conv(
            in_c, in_c, kernel_size=7, stride=1, padding=3, groups=in_c
        )
...
class GNConvLayer(OGConvLayer):
    def __init__(
        ...
        # Category embedding
        self.mlp_cat = (
            nn.Sequential(nn.Linear(cat_emb_dim, 2 * out_c))
            if cat_emb_dim is not None
            else None
        )
